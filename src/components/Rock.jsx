...
const FloatingLogo = ({ position = [0, 1, 2], logoUrl }) => {
  ...
  useFrame(() => {
    if (meshRef.current) {
      ...
      meshRef.current.position.y = Math.sin(time + position[1])* -8.5 + position[0];
  ...
  return (
    <>
      ...
      {showInfo && (
        <Html position={[position[0], position[1] + 1, position[1] +1]}>
          <div style={{
